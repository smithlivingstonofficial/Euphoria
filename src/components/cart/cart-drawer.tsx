    ...
    for (let i = 0; i < selectedEvents.length; i++) {
      for (let j = i + 1; j < selectedEvents.length; j++) {
        ...
        if (e1.event_date === e2.event_date && e1.event_date) {
          // Check if timings match or overlap
            ...
            {successResult ? (
              ...
              <div className="text-center py-16 space-y-3">
                ...
                <p className="text-xs text-slate-500 max-w-xs mx-auto">
                  Explore all 61 competitions and click &quot;Select Event&quot; to build your personalized Euphoria schedule.
                </p>
                <button
                  onClick={closeCart}
              ...
              <>
                ...
                <div className="space-y-3">
                  {selectedEvents.map((evt, index) => (
                    <div
                      key={evt.id}
                      className="group relative rounded-2xl border border-slate-200/90 bg-white p-3.5 shadow-2xs hover:shadow-xs transition-all flex items-start justify-between gap-3"
                    >
                ...
                {/* Pricing & Allocation Breakdown Card */}
                <div className="rounded-2xl border border-slate-200 bg-slate-50/90 p-4 space-y-3 text-xs">
                  ...
                  <div className="space-y-1.5 text-[11px] text-slate-600">
                    <div className="flex items-center justify-between">
                      <span>Base Pass Fee (includes {pricing.isInternal ? "3" : "2"} events)</span>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-bold text-slate-900">
                        {selectedEvents.length} Events ({pricing.includedCount} Included
                        {pricing.extraEventsCount > 0 ? ` + ${pricing.extraEventsCount} Extra` : ""})
                      </span>
                    </div>

                    {pricing.extraEventsCount > 0 && (
                      <div className="flex items-center justify-between text-indigo-700 font-semibold">
                        <span>
                          Extra Events Charge ({pricing.extraEventsCount} ×{" "}
                          {formatCurrency(pricing.isInternal ? 100 : 150)})
                        </span>
                        <span>+{formatCurrency(pricing.extraFee)}</span>
                      </div>
                    )}
                  </div>
              ...
              >
                {isSubmitting ? (
                  ...
                  <>
                    ...
                    <span>
                      Register All {selectedEvents.length} Events • {formatCurrency(pricing.totalAmount)}
                    </span>
